...
from llava.train.llava_trainer import LLaVATrainerSteer
...
@dataclass
class DataArguments:
    data_path: str = field(default=None, metadata={"help": "Path to the harmful training data, in llava's instruction.json format. Supporting multiple json files via /path/to/{a,b,c}.json"})
    data_path2: str = field(default=None, metadata={"help": "Path to the harmless training data, in llava's instruction.json format. Supporting multiple json files via /path/to/{a,b,c}.json"})
...
class LazySupervisedDataset(Dataset):
    def __init__(self, data_path: str,data_path2: str, tokenizer: transformers.PreTrainedTokenizer, data_args: DataArguments):
        super(LazySupervisedDataset, self).__init__()
        self.tokenizer = tokenizer
        self.list_data_dict = []
        self.list_data_dict2 = []
        ...
        if "{" in data_path2 and "}" in data_path2:
            base_path, file_pattern = re.match(r"^(.*)\{(.*)\}\.json$", data_path2).groups()
            file_names = file_pattern.split(",")
            rank0_print(f"Loading {file_names} from {base_path}")
            data_args.dataset_paths = []
            for file_name in file_names:
                ...
                with open(full_path, "r") as file:
                    cur_data_dict = json.load(file)
                    rank0_print(f"Loaded {len(cur_data_dict)} samples from {full_path}")
                    self.list_data_dict2.extend(cur_data_dict)
        elif data_path2.endswith(".yaml"):
            ...
            data_args.dataset_paths = [data_path2]
            rank0_print(f"Loading {data_path2}")
            with open(data_path2, "r") as file:
                cur_data_dict = json.load(file)
                rank0_print(f"Loaded {len(cur_data_dict)} samples from {data_path2}")
                self.list_data_dict2.extend(cur_data_dict)

        rank0_print(f"Loaded {len(self.list_data_dict2)} samples from {data_path2}")
        rank0_print("Formatting inputs...Skip in lazy mode")
        self.tokenizer = tokenizer
        self.data_args = data_args

    def __len__(self):
        return min(len(self.list_data_dict),len(self.list_data_dict2))
    ...
    def process_image(self, image_file, overwrite_image_aspect_ratio=None,loaded_image=None):
    ...
    def _get_item(self, i) -> Dict[str, torch.Tensor]:
        ...
        ###
        sources = self.list_data_dict2[i]
        if isinstance(i, int):
            sources = [sources]
        assert len(sources) == 1, "Don't know why it is wrapped to a list"  # FIXME

        if "image" in sources[0]:
            image_file = self.list_data_dict2[i]["image"]
            if type(image_file) is list:
                image = [self.process_image(f) for f in image_file]
                # Handling multi images
                # overwrite to process with simple pad
            ...
            video_file = self.list_data_dict2[i]["video"]
            video_folder = self.data_args.video_folder
            video_file = os.path.join(video_folder, video_file)
            suffix = video_file.split(".")[-1]
            if not os.path.exists(video_file):
        ...
        has_image = ("image" in self.list_data_dict2[i]) or ("video" in self.list_data_dict2[i])
        data_dict2 = preprocess(sources, self.tokenizer, has_image=has_image)

        if "prompt" in data_dict:
            prompt = data_dict2["prompt"]
        else:
            prompt = None

        if isinstance(i, int):
            data_dict2 = dict(input_ids2=data_dict2["input_ids"][0], labels2=data_dict2["labels"][0])

        # image exist in the data
        if "image" in self.list_data_dict2[i]:
            data_dict2["image2"] = image
        elif "video" in self.list_data_dict2[i]:
            data_dict2["image2"] = image
        elif self.data_args.is_multimodal:
            # image does not exist in the data, but the model is multimodal
            crop_size = self.data_args.image_processor.crop_size
        ...
        # prompt exist in the data
        if prompt is not None:
            data_dict2["prompt2"] = prompt

        data_dict2["id2"] = self.list_data_dict2[i].get("id", i)

        merged_dict = {**data_dict, **data_dict2}
...
####
        input_ids, labels = tuple([instance[key] for instance in instances] for key in ("input_ids2", "labels2"))
        # input_ids, labels, ids = tuple([instance[key] for instance in instances] for key in ("input_ids", "labels", "id"))
        input_ids = [_input_ids[: self.tokenizer.model_max_length] for _input_ids in input_ids]
        labels = [_labels[: self.tokenizer.model_max_length] for _labels in labels]
        if self.tokenizer.pad_token_id is None:
        ...
        input_ids = self.pad_sequence(input_ids, batch_first=True, padding_value=self.tokenizer.pad_token_id)
        labels = self.pad_sequence(labels, batch_first=True, padding_value=IGNORE_INDEX)
        batch2 = dict(input_ids2=input_ids, labels2=labels.long() if labels.dtype == torch.int32 else labels,
                     attention_mask2=input_ids.ne(self.tokenizer.pad_token_id))
        # batch = dict(input_ids=input_ids, labels=labels, attention_mask=input_ids.ne(self.tokenizer.pad_token_id), ids=ids)
        ...
        if "image" in instances[0]:
            ...
            batch2["image_sizes2"] = [im[1] for im_list in images2 for im in im_list]
            batch2["modalities2"] = [im[2] for im_list in images2 for im in im_list]
            images2 = [im[0] for im_list in images2 for im in im_list]
            ...
            #     batch["images"] = torch.stack(images)
            # else:
            batch2["images2"] = images2

        if "prompt" in instances[0]:
            batch["prompts"] = [instance["prompt"] for instance in instances]
            batch2["prompts2"] = [instance["prompt2"] for instance in instances]
        merge_batch = {**batch, **batch2}
        return merge_batch


def make_supervised_data_module(tokenizer: transformers.PreTrainedTokenizer, data_args) -> Dict:
    """Make dataset and collator for supervised fine-tuning."""
    train_dataset = LazySupervisedDataset(tokenizer=tokenizer, data_path=data_args.data_path,data_path2=data_args.data_path2, data_args=data_args)
...
def train(attn_implementation=None):
    ...
    trainer = LLaVATrainerSteer(model=model, tokenizer=tokenizer, args=training_args, **data_module)
